...
//
// LOCAL USAGE:
// Wishbone UART core wrapped by qosoc_uart.
//
// NOTE:
...
module wbuart (
  ...
  // use flow control.
  output reg o_rts_n;
  output wire o_uart_rx_int, o_uart_tx_int, o_uart_rxfifo_int, o_uart_txfifo_int;
  ...
  //
  reg [30:0] uart_setup;
  reg r_wb_ack;  // Moved from below
  always @(posedge clk_i)
    if (rst_i) begin
      uart_setup <= INITIAL_SETUP | ((HARDWARE_FLOW_CONTROL_PRESENT == 1'b0) ? 31'h40000000 : 0);
    end else if ((i_wb_stb) && (i_wb_addr == `UART_SETUP) && (i_wb_we))
...
`ifdef USE_LITE_UART
  rxuartlite #(INITIAL_SETUP[23:0]) rx (
      clk_i,
      i_uart_rx,
      rx_stb,
      rx_uart_data
  ...
  always @(posedge clk_i) begin
    if (rst_i) begin
      ...
      txf_wb_write <= (i_wb_stb)&&(i_wb_addr == `UART_TXREG)
                    &&(i_wb_we)&&(!r_wb_ack); // Block double-writes
...
`ifndef USE_LITE_UART
  ...
  reg r_tx_break;
  always @(posedge clk_i)
    if (rst_i) r_tx_break <= 1'b0;
    else if ((i_wb_stb) && (i_wb_addr[1:0] == `UART_TXREG) && (i_wb_we)) r_tx_break <= i_wb_data[9];
...
`endif
  ...
  // normal.
  always @(posedge clk_i)
    if (rst_i) tx_uart_reset <= 1'b1;
    else if ((i_wb_stb) && (i_wb_addr == `UART_SETUP) && (i_wb_we)) tx_uart_reset <= 1'b1;
    else if ((i_wb_stb) && (i_wb_addr[1:0] == `UART_TXREG) && (i_wb_we))
      tx_uart_reset <= i_wb_data[12];
    else tx_uart_reset <= 1'b0;

`ifdef USE_LITE_UART
  txuartlite #(INITIAL_SETUP[23:0]) tx (
      clk_i,
      (tx_empty_n),
      tx_data,
      o_uart_tx,
...
`endif
  ...
  assign wb_tx_data = {
    ...
    (tx_busy | txf_status[0]) ? txf_wb_data : 8'b00000000
  };
  ...
  wire [33:0] unused;
  assign unused = {rst_i, i_wb_cyc, i_wb_data};
