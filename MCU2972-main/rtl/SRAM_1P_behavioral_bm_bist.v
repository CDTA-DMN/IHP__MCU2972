...
//
// LOCAL USAGE:
// Behavioral SRAM model used for functional simulation and GLS support.
//
// NOTE:
...
module SRAM_1P_behavioral_bm_bist (
    ...
    parameter integer P_ADDR_WIDTH = 9;
    ...
    reg  [P_DATA_WIDTH-1:0] memory   [0:(1 << P_ADDR_WIDTH)-1];  // memory
    reg  [P_DATA_WIDTH-1:0] dr_r;
    
    // Initialize memory to prevent X propagation in GLS
    integer i;
    initial begin
        for (i = 0; i < (1 << P_ADDR_WIDTH); i = i + 1) begin
            memory[i] = {P_DATA_WIDTH{1'b0}};
        end
        dr_r = {P_DATA_WIDTH{1'b0}};
    end
