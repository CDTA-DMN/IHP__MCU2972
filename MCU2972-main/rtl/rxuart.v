...
//
// LOCAL USAGE:
// wbuart receive engine used by wbuart integration module.
//
// NOTE:
...
module rxuart (
  ...
  reg half_baud_time;
  always @(posedge clk_i) 
    if (rst_i) half_baud_time <= 0;
    else half_baud_time <= (~ck_uart) && (chg_counter >= half_baud);
  ...
  always @(posedge clk_i)
    if (rst_i) o_wr <= 1'b0;
    else if ((zero_baud_counter) || (state == `RXU_IDLE)) o_wr <= (pre_wr);
    else o_wr <= 1'b0;
  ...
  // intervals.
  always @(posedge clk_i)
    if (rst_i) baud_counter <= clocks_per_baud - 28'h01;
  ...
  always @(posedge clk_i)
    if (rst_i) zero_baud_counter <= 1'b0;
    else if (state == `RXU_IDLE) zero_baud_counter <= 1'b0;
