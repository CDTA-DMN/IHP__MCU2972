...
//
// LOCAL USAGE:
// wbuart transmit engine used by wbuart integration module.
//
// NOTE:
...
module txuart (
  ...
  // initial    ck_cts   = 1'b0;
  always @(posedge clk_i) {qq_cts_n, q_cts_n} <= {q_cts_n, i_cts_n};
  always @(posedge clk_i) ck_cts <= (!qq_cts_n) || (!hw_flow_control);
  ...
  always @(posedge clk_i)
    if (rst_i) begin
      ...
      state  <= TXU_BREAK;
      r_busy <= 1'b1;
    end else if (!zero_baud_counter) begin  // r_busy needs to be set coming into here
      r_busy <= 1'b1;
    ...
    begin
      if ((i_wr) && (!r_busy)) begin  // Immediately start us off with a start bit
        r_busy <= 1'b1;
        case (i_data_bits)
      ...
      if (state[3] == 0) // First 8 bits
        ...
        state <= TXU_STOP;
      end else if (state == TXU_STOP) begin  // two stop bit(s)
  ...
  always @(posedge clk_i)
    if (rst_i) lcl_data <= 8'hff;
    else if (!r_busy) lcl_data <= i_data;
  ...
  always @(posedge clk_i)
    if (rst_i) o_uart_tx <= 1'b1;
    else if ((i_break) || ((i_wr) && (!r_busy))) o_uart_tx <= 1'b0;
    else if (zero_baud_counter)
      casez (state)
        4'b0000???: o_uart_tx <= lcl_data[0];
  ...
  always @(posedge clk_i)
    if (rst_i) calc_parity <= 1'b0;
    else if (!o_busy) calc_parity <= i_setup[24];
  ...
  always @(posedge clk_i)
    if (rst_i) last_state <= 1'b0;
    else if (dblstop) last_state <= (state == TXU_SECOND_STOP);
...
`ifdef FORMAL
  ...
  always_comb assert (r_setup == fsv_setup);


  always @(posedge i_clk) assert (zero_baud_counter == (baud_counter == 0));

  always_comb if (!o_busy) assert (zero_baud_counter);
  ...
  always @(posedge i_clk)
    ...
    else if ((state == TXU_IDLE) && (i_wr) && (!o_busy) && (i_data_bits == 2'b10))  // six data bits
      f_six_seq <= 1;
      ...
      assert (f_counter == (fsv_setup[23:0] - baud_counter - 1));
        ...
        assert (f_counter == ({4'h0, fsv_setup[23:0], 1'b0} - baud_counter - 1));
          ...
          assert(f_counter == ({4'h0, fsv_setup[23:0], 1'b0}
            ...
            assert (f_counter == ({3'h0, fsv_setup[23:0], 2'b00} - baud_counter - 1));
            else if (f_five_seq[4] | f_six_seq[4] | f_seven_seq[4] | f_eight_seq[4])
              assert(f_counter == ({3'h0, fsv_setup[23:0], 2'b00}
                +{5'h0, fsv_setup[23:0]}
                - baud_counter - 1));
                ...
                - baud_counter - 1));
                    ...
                    assert (f_counter == ({2'h0, fsv_setup[23:0], 3'b000}  // 8
                    - baud_counter - 1));
                    else if (f_eight_seq[8])
                      assert (f_counter == ({2'h0, fsv_setup[23:0], 3'b000}  // 9
                      + {5'h0, fsv_setup[23:0]} - baud_counter - 1));
                      else if (f_stop_seq[0] || (!use_parity && f_stop_seq[1])) begin
                        // Parity bit, or first of two stop bits
                        case (data_bits)
                          2'b00:
                          assert(f_counter == ({2'h0, fsv_setup[23:0], 3'b000}
                ...
                +{5'h0, fsv_setup[23:0]}    // 7
                          + {4'h0, fsv_setup[23:0], 1'b0} - baud_counter - 1));
                          default: ; // lint: case-missing-default
                        endcase
                ...
                - baud_counter - 2)); // 8
                          ...
                          assert(f_counter == ({3'h0, fsv_setup[23:0], 2'b00}
                +{5'h0, fsv_setup[23:0]}    // 7
                          + {4'h0, fsv_setup[23:0], 1'b0} - baud_counter - 2));
                          default: ; // lint: case-missing-default
                        endcase
...
`endif
  ...
  //
  //////////////////////////////////////////////////////////////////////
  always_comb assert ((state < 4'hb) || (state >= 4'he));
  //////////////////////////////////////////////////////////////////////
  //
  // Careless/limiting assumption section
  //
  //////////////////////////////////////////////////////////////////////
  always_comb assume (i_setup[23:0] > 2);
  always_comb assert (fsv_setup[23:0] > 2);
