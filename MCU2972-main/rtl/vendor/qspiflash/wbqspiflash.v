...
//
// LOCAL USAGE:
// Wishbone-facing QSPI flash controller wrapped by qosoc_qspi.
//
// NOTE:
...
module	wbqspiflash #(
		...
		parameter [0:0]	OPT_WINBOND = 1'b1,  // Set to 1 for Winbond W25Q, 0 for Micron N25Q
		localparam	AW = ADDRESS_WIDTH-2
		// }}}
	) (
		// {{{
		input	wire		clk_i,
		input	wire		rst_i,  // Added reset input for GLS compatibility
	...
	// Erase status tracking (moved to reset logic)
...
begin
    if (!trace_ctrl)
        trace_ctrl <= $test$plusargs("trace_wbqspi");

    if (trace_ctrl) begin
        if (state_prev != state) begin
            $display("[WBQSPI] state %0d -> %0d reset_cnt=%0d @ %t", state_prev, state, reset_counter, $time);
            state_prev <= state;
        end

        if (spi_wr && (spi_in_prev != spi_in)) begin
            $display("[WBQSPI] spi_wr cmd=%02x len=%0d quad=%b cs=%b @ %t", spi_in[31:24], spi_len, quad_mode_enabled, o_qspi_cs_n, $time);
            spi_in_prev <= spi_in;
        end
    end
end
`endif
	...
	begin
		if (rst_i) begin
			// Reset all registers to known state for GLS compatibility
			state <= `WBQSPI_RESET;
			o_wb_ack <= 1'b0;
			o_wb_stall <= 1'b1;
			spi_wr <= 1'b0;
			...
			write_protect <= 1'b1; // Default to protected
			erased_sector <= 0;
			dirty_sector <= 1'b1;
			spif_addr <= 0;
			o_wb_data <= 32'h0;
...
`endif
		...
		begin
			...
			begin // Read access, normal mode(s)
				...
				begin
					...
					spi_spd    <= 1'b0;  // Single SPI for command
					spi_dir    <= 1'b0;  // Write (output)
				end else begin
	...
	begin
		...
		begin
			// Control register write (unlock, erase, etc) - need to exit quad mode
			o_wb_ack   <= 1'b0;
			o_wb_stall <= 1'b1;
			spi_wr <= 1'b1;
			spi_len <= 2'b11;
		...
		begin // Control register read (not addr 0) - need to exit quad mode
			o_wb_ack   <= 1'b0;
			o_wb_stall <= 1'b1;
			spi_wr <= 1'b1;
			spi_len <= 2'b11;
			spi_in <= 32'h00;
			state <= `WBQSPI_WBDECODE;
		end else if((i_wb_data_stb))
		begin // Data access that's not a read continuation - exit quad mode
			o_wb_ack   <= 1'b0;
			o_wb_stall <= 1'b1;
			spi_wr <= 1'b1;
			spi_len <= 2'b11;
...
//
	...
	begin
		...
		begin // Single pulse acknowledge and write data out
			o_wb_ack <= spif_req;
			o_wb_stall <= (!spi_wr);
			// Swap bytes for correct little-endian order
			o_wb_data <= { spi_out[7:0], spi_out[15:8], spi_out[23:16], spi_out[31:24] }; 
	...
	begin
		...
		begin // Put the results out as soon as possible
			// Byte-swap to match little-endian CPU expectations
			o_wb_data <= { spi_out[7:0], spi_out[15:8], spi_out[23:16], spi_out[31:24] };
			o_wb_ack <= spif_req;
			spif_req <= 1'b0;
...
//
	...
	begin // We enter immediately after commanding a WEN
		...
		if (OPT_WINBOND) begin
			// Winbond: Write Status Register-2 using command 0x31
			spi_len <= 2'b00;  // 8 bits (command + data)
			spi_in <= { 8'h31, spif_data[7:0], 16'h00 };
		end else begin
			// Micron: Write Status + Config together using command 0x01
			spi_len <= 2'b10;  // 24 bits
	...
	begin // We enter immediately after commanding a WEN
		...
		// For both Winbond and Micron: Write Status Register-1 using command 0x01
		spi_len <= 2'b00;  // Changed from 2'b01 - only 8 bits for Winbond compatibility
		...
		begin
			...
			write_in_progress <= 1'b1;
			if (OPT_WINBOND) begin
				// Winbond doesn't need CLEAR_STATUS
				state <= `WBQSPI_WAIT_TIL_IDLE;
			end else begin
				// Micron: Check if we need to clear status bits
	...
	begin // Know that WIP is clear on entry, WEN has just been commanded
		...
		// Here's the erase command (0xD8 = 64KB Block Erase)
		// Support full 24-bit address range from spif_data[23:0]
		spi_in <= { 8'hd8, spif_data[23:0] };
		spi_len <= 2'b11; // 32 bit write (8-bit cmd + 24-bit addr)
		// together with setting our copy of the WIP bit
		write_in_progress <= 1'b1;
		// keeping track of which sector we just erased
		// Sector defined by bits 23:14 (byte addr bit 16 is 64KB)
		erased_sector <= spif_data[23:16];
	...
	end  // End of always @(posedge i_clk) block
	...
	assign	o_qspi_mod  = (spif_override)?  2'b00 :w_qspi_mod;
