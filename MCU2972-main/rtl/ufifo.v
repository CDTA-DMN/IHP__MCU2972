...
//
// LOCAL USAGE:
// Shared FIFO primitive used by wbuart for RX/TX buffering.
//
// NOTE:
...
module ufifo (
  ...
  parameter int BW = 8;  // Byte/data width
  parameter logic [3:0] LGFLEN = 4;
  parameter int RXFIFO = 1'b0;
  ...
  localparam int FLEN = (1 << LGFLEN);

  reg [(BW-1):0] fifo[FLEN];
  reg [(LGFLEN-1):0] r_first, r_last, r_next;

  wire [(LGFLEN-1):0] w_first_plus_one, w_first_plus_two, w_last_plus_one;
  ...
  reg will_overflow;
  // removed initial will_overflow = 1'b0;
  always @(posedge i_clk) begin
    if (i_rst) will_overflow <= 1'b0;
    else if (i_rd) will_overflow <= (will_overflow) && (i_wr);
    else if (i_wr) will_overflow <= (will_overflow) || (w_first_plus_two == r_last);
    else if (w_first_plus_one == r_last) begin
        will_overflow <= 1'b1;
    end
  end
  ...
  // Write
  reg r_ovfl;
  // removed initial r_first = 0;
  // removed initial r_ovfl = 0;
  ...
  always @(posedge i_clk) begin
    if (i_wr || i_rd) begin
        // $display("[UFIFO] %m: wr=%b, rd=%b, first=%d, last=%d, next=%d, will_ovfl=%b, will_unfl=%b at %t", 
        //          i_wr, i_rd, r_first, r_last, r_next, will_overflow, will_underflow, $time);
    end
  ...
  //    7    0    3        fifo[3]
  reg will_underflow;
  // removed initial will_underflow = 1'b1;
  always @(posedge i_clk)
    if (i_rst) will_underflow <= 1'b1;
    else if (i_wr) will_underflow <= 1'b0;
    else if (i_rd) will_underflow <= (will_underflow) || (w_last_plus_one == r_first);
    else will_underflow <= (r_last == r_first);
  ...
  //
  // reg        r_unfl;
  // removed initial r_unfl = 1'b0;
  // removed initial r_last = 0;
  // removed initial r_next = {{(LGFLEN - 1) {1'b0}}, 1'b1};
  always @(posedge i_clk)
    if (i_rst) begin
      r_last <= 0;
  ...
  always @(posedge i_clk) fifo_here <= fifo[r_last];
  always @(posedge i_clk) fifo_next <= fifo[r_next];
  always @(posedge i_clk) r_data <= i_data;
  ...
  always @(posedge i_clk)
    if (will_underflow)
      // o_data <= i_data;
      osrc <= 2'b00;
    else if ((i_rd) && (r_first == w_last_plus_one)) osrc <= 2'b01;
    else if (i_rd) osrc <= 2'b11;
    else osrc <= 2'b10;
  ...
  reg r_empty_n;
  // removed initial r_empty_n = 1'b0;
  always @(posedge i_clk)
    if (i_rst) r_empty_n <= 1'b0;
    else
      casez ({
        i_wr, i_rd, will_underflow
      })
        3'b000?: r_empty_n <= (r_first != r_last);
        3'b010: r_empty_n <= (r_first != w_last_plus_one);
        3'b010?: r_empty_n <= 1'b1;
  ...
  /* generate
    if (RXFIFO != 0) initial r_fill = 0;
    else initial r_fill = -1;
  endgenerate */

  always @(posedge i_clk)
    if (i_rst) begin
       if (RXFIFO != 0) r_fill <= 0;
       else r_fill <= {(LGFLEN){1'b1}};
    end else if (RXFIFO != 0) begin
      ...
      if (i_rst) r_fill <= 0;
        ...
        casez ({
          (i_wr), (!will_overflow), (i_rd) && (!will_underflow)
        })
          3'b000?1:  r_fill <= r_first - r_next;
          3'b110:  r_fill <= r_first - r_last + 1'b1;
          3'b001?1:  r_fill <= r_first - r_last;
      ...
      if (i_rst) r_fill <= {(LGFLEN) {1'b1}};
      else
        casez ({
          i_wr, (!will_overflow), (i_rd) && (!will_underflow)
        })
          3'b000?1:  r_fill <= r_fill + 1'b1;
  ...
  generate
    if (LGFLEN < 10) assign w_fill[9:(LGFLEN)] = 0;
  endgenerate
    ...
    if (f_past_valid) begin
      if ($past(i_rst))
        assert (!o_err);
          ...
          if (($past(i_wr)) && (!$past(i_rd)) && ($past(will_overflow))) assert (o_err);
        end
    end
